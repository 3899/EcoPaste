    ...
    win32wm::WM_PAINT => {
      ...
      if subclass_input.event_loop_runner.handling_events() {
        if subclass_input.event_loop_runner.should_buffer() {
          ...
          // EcoPaste patch:
          // In practice this can be re-entrant on Windows and the upstream assert
          // aborts the whole app. If flushing reports re-entry, keep the event
          // loop alive and ask Windows to schedule another internal paint pass.
