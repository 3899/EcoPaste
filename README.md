...
- **☁️ WebDAV 云端备份**：支持通过 WebDAV 协议将剪贴板数据备份到云端（坚果云、NextCloud 等），提供精简备份模式（跳过图片和文件仅保留文本等内容）、自动定时备份、备份数量限制、一键恢复等完整功能。
- **🎨 独立群组与颜色直出**：新增 "链接"、"颜色"、"代码"、"邮箱" 原生专属群组分类。精准提取并高亮显示 RGB/RGBA 等颜色格式，路径链接高亮直击目标。
- **📝 丰富二次沉浸编辑**：支持文本等富内容数据的独立二级弹窗编辑与系统级别快速文件定位。
...
- **️ 智能删除确认**：删除图片内容时，二次确认弹窗增加"删除本地文件"复选框（默认选中），可选择保留本地文件仅删除剪贴板记录。

### 📏 文本与图片显示高级适配

- **文本/代码/文件显示行数自定义**：偏好设置 → 剪贴板 → 显示设置 → 文本/代码/文件 显示行数（1 - 50 行）
...
- **⚙️ 配置持久化**：所有新增配置项自动保存到用户数据目录，应用更新后配置不会丢失，无需手动导出/导入。
