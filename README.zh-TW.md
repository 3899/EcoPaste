...
- **☁️ WebDAV 雲端備份**：支援透過 WebDAV 協議將剪貼板資料備份到雲端（堅果雲、NextCloud 等），提供精簡備份模式（跳過圖片和檔案僅保留文字等內容）、自動定時備份、備份數量限制、一鍵恢復等完整功能。
...
- **🗑️ 智能刪除確認**：刪除圖片內容時，二次確認彈窗增加"刪除本地檔案"複選框（預設選中），可選擇保留本地檔案僅刪除剪貼板記錄。

### 📏 文字與圖片顯示高級適配

- **文字/代碼/檔案顯示行數自訂**：偏好設定 → 剪貼板 → 顯示設定 → 文字/代碼/檔案 顯示行數（1 - 50 行）
...
- **📸 截圖完美轉儲**：重構 SQLite 日誌行為及底層 FS 映射鏈路，讓截圖圖片完美儲存至指定的自訂本地目錄，徹底解決原版截圖因內建庫限制導致自訂目錄崩潰和展示圖片白板紅叉的問題，杜絕 C 槽資源浪費。
...
- **⚙️ 配置持久化**：所有新增配置項自動儲存到用戶資料目錄，應用更新後配置不會丟失，無需手動匯出/匯入。
